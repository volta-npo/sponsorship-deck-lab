...
import { buildSaasExpansionSuite, buildSaasSuiteMarkdown } from './saas-suite.js';
...
function install() {
    ...
    <div class="domain-head"><div><p class="eyebrow">Standalone SaaS product suite</p><h2 id="domain-title">${esc(domain.title)}</h2><p class="muted">${esc(domain.purpose)}</p></div><div class="button-row no-print"><button id="domain-sample" class="secondary">Load Domain Sample</button><button id="suite-export" class="secondary">Export SaaS Suite</button><button id="domain-export">Export Tool Packet</button></div></div>
    ...
    <div id="saas-suite" class="artifact" aria-live="polite"></div>
    ...
    $('#suite-export').addEventListener('click', () => download(`${config.slug}-saas-suite.md`, buildSaasSuiteMarkdown(config, domain, state), 'text/markdown'));
    render();
}
function render() {
    ...
    $('#saas-suite').innerHTML = `<strong>SaaS suite depth</strong><p>${suite.modules.length} modules, ${suite.workflows.length} workflows, ${suite.records.length} operating records, ${suite.automations.length} automations, ${suite.exports.length} export products.</p><p>${esc(suite.modules.slice(0, 5).join(' • '))}</p>`;
